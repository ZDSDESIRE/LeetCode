...
如果 A[i - 1] < A[i]，则区间 \big[A[i - 1] + 1, A[i] - 1\big][A[i−1]+1,A[i]−1] 里的数都是没有出现过的，所以我们可以将之前重复的数变为这个区间范围内的数。设当前重复的数的个数为 taken，则我们最多可以改变 give = min(taken, A[i] - A[i - 1] - 1) 个数，即区间 \big[A[i - 1] + 1, A[i] - 1\big][A[i−1]+1,A[i]−1] 的长度与 taken 二者的较小值。
