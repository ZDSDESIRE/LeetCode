...
我们的复杂度分析依赖于理解 generateParenthesis(n) 中有多少个元素。这个分析超出了本文的范畴，但事实证明这是第 n 个卡特兰数 (1/n+1)(2n-n)，这是由 4^n / √n 渐近界定的。

- 时间复杂度：O(4^n / √n)，在回溯过程中，每个答案需要 O(n) 的时间复制到答案数组中。
